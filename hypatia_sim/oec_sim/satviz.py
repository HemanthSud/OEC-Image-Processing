...
<body>
  ...
  <script>
    ...
    // Synchronous primitives (not entities): entity geometry batches lazily
    // per frame and is starved by the per-frame link updates below — the
    // continents could take a minute to appear. asynchronous:false
    // triangulates once at load and renders on the first frame.
    var landFill = Cesium.Color.fromCssColorString('#b9c4cc');
    var coastCol = Cesium.Color.fromCssColorString('#6e7a85');
    var landInstances = [], coastInstances = [];
    D.land.forEach(function(poly) {
      // Cesium triangulates the Antarctic rings (mainland encloses the pole,
      // ice shelves hug it) inside-out, tinting the entire ocean — draw
      // everything south of 60°S as coastline only.
      var latMin = 90;
      for (var k = 1; k < poly.o.length; k += 2)
        if (poly.o[k] < latMin) latMin = poly.o[k];
      if (latMin > -60) {
        landInstances.push(new Cesium.GeometryInstance({
          ...
          attributes: { color:
            Cesium.ColorGeometryInstanceAttribute.fromColor(landFill) }
        }));
      }
      coastInstances.push(new Cesium.GeometryInstance({
        ...
        attributes: { color:
          Cesium.ColorGeometryInstanceAttribute.fromColor(coastCol) }
      }));
    });
    ...
    scene.primitives.add(new Cesium.Primitive({
      geometryInstances: coastInstances,
