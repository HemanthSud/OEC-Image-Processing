...
Random seed & 42 (task generation; sensitivity sweeps use 5 seeds) \\
...
(Table~\ref{tab:regimes}). \textbf{gbs-limited} is the default; its per-GBS
aggregate budget is $\ge 12\times$ tighter than any ISL segment a route
could cross, so ISL contention is mathematically unreachable there.
\textbf{fabric-limited} throttles the ISL rate specifically so routing
decisions have a measurable effect. Both regimes are used across the
scheduler-comparison and decision-distribution results this setup feeds,
...
The utility function's quality term $s_q$ (\S\ref{sec:formulation}) is
grounded in measured downstream FLAIR-1 segmentation mIoU, source label
\texttt{flair-unet-r34-rgbie / val7050 / 2026-08-31} (full 7{,}050-image
validation population; the measurement itself is written up separately).
Every scheduler result this setup feeds uses this real, measured table
(\texttt{hypatia\_sim/oec\_sim/quality\_table.json}), not the placeholder
fallback (\texttt{config.QUALITY\_TABLE\_FALLBACK}) that the code falls
back to when this file is absent.
