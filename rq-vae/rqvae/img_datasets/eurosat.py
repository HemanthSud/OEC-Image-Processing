...
class EuroSAT(Dataset):
    ...
    def __init__(self, root, split='train', transform=None, max_samples=None,
                 split_indices_path=None, return_label=False):
        """
        Args:
            ...
            return_label: If True, return actual class label instead of dummy 0.
        """
        self.root = root
        self.split = split
        ...
        if split_indices_path is not None and split != 'all':
            import torch
            indices = torch.load(split_indices_path)
            if split in ('train',):
        ...
        self.labels = [EUROSAT_CLASSES.index(os.path.basename(os.path.dirname(f)))
                       for f in self.files]
    ...
    def __getitem__(self, idx):
        ...
        label = self.labels[idx] if self.return_label else 0
        return (img, label)
